...
import * as thresholdsModule from '@delegates/utils/thresholds';
...
export default () => {
  describe('setup-card.ts', () => {
    ...
    let hitThresholdsStub: sinon.SinonStub;

    beforeEach(() => {
      // Create stub for hitThresholds
      hitThresholdsStub = sinon.stub(thresholdsModule, 'hitThresholds');
    ...
    afterEach(() => {
      hitThresholdsStub.restore();
    });

    it('should return all required properties with basic config', () => {
      // Set up hitThresholds stub to return specific values
      hitThresholdsStub.returns({
        overTemp: false,
        overHumid: true,
      });
      ...
      // Verify hitThresholds was called and values returned
      expect(hitThresholdsStub.calledOnce).to.be.true;
      expect(result.overTemp).to.be.false;
      expect(result.overHumid).to.be.true;
    });
