...
import { BadgeEditorManager } from '../../../../../src/cards/components/editor/utils/badge-editor-manager';

describe('BadgeEditorManager', () => {
  describe('createNewBadge', () => {
    it('should create a new badge with default position', () => {
      const badge = BadgeEditorManager.createNewBadge();
    ...
    it('should create a new badge instance each time', () => {
      const badge1 = BadgeEditorManager.createNewBadge();
      const badge2 = BadgeEditorManager.createNewBadge();
  ...
  describe('addBadge', () => {
    it('should add new badge to empty array', () => {
      ...
      const newBadges = BadgeEditorManager.addBadge(badges);
    ...
    it('should handle undefined badges array', () => {
      const newBadges = BadgeEditorManager.addBadge(undefined);
      expect(newBadges).to.have.lengthOf(1);
      expect(newBadges[0]).to.deep.equal({
        position: 'top_right',
      });
  ...
  describe('adjustExpandedIndicesAfterRemoval', () => {
    it('should remove the deleted index from expanded states', () => {
      ...
      const result = BadgeEditorManager.adjustExpandedIndicesAfterRemoval(
        expandedBadges,
        1,
      );
      // Removing index 1:
  ...
  describe('removeBadgeItem', () => {
    it('should remove badge at specified index', () => {
      ...
      const newBadges = BadgeEditorManager.removeBadgeItem(badges, 1);
      expect(newBadges).to.have.lengthOf(2);
      expect(newBadges[0]).to.deep.equal(badges[0]);
    ...
    it('should remove first badge', () => {
      ...
      const newBadges = BadgeEditorManager.removeBadgeItem(badges, 0);
      expect(newBadges).to.have.lengthOf(1);
    ...
    it('should handle undefined badges array', () => {
      const newBadges = BadgeEditorManager.removeBadgeItem(undefined, 0);
      expect(newBadges).to.be.an('array');
      expect(newBadges.length).to.equal(0);
    });
  ...
  describe('integration', () => {
    it('should handle complete workflow: add, remove, adjust indices', () => {
      ...
      let badges = BadgeEditorManager.addBadge(undefined);
      expect(badges).to.have.lengthOf(1);

      // Add more badges
      badges = BadgeEditorManager.addBadge(badges);
      badges = BadgeEditorManager.addBadge(badges);
      ...
      // Remove middle badge
      badges = BadgeEditorManager.removeBadgeItem(badges, 1);
      expandedBadges = BadgeEditorManager.adjustExpandedIndicesAfterRemoval(
        expandedBadges,
        1,
      );
    ...
    it('should handle removing all badges', () => {
      ...
      badges = BadgeEditorManager.removeBadgeItem(badges, 0);
      badges = BadgeEditorManager.removeBadgeItem(badges, 0);
