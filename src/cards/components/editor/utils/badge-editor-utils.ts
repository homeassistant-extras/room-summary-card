...
/**
 * Utility functions for badge row editor
 */
export class BadgeEditorUtils {
  /**
   * Generates a unique key for a badge item in a list
   */
  static getKey(item: BadgeConfig, index: number): string {
    return `badge-${index}`;
  }
   ...
   */
  static cleanEmptyStrings(obj: any): any {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        const cleanedValue = BadgeEditorUtils.cleanEmptyStrings(value);
        if (Object.keys(cleanedValue).length > 0) {
          cleaned[key] = cleanedValue;
        }
      } else if (Array.isArray(value)) {
        const cleanedArray = value
          .map((item) => BadgeEditorUtils.cleanEmptyStrings(item))
          .filter((item) => item !== '');
   ...
   */
  static getBadgeTitle(badge: BadgeConfig): string {
